...
- **신규콜 시트 (Google Sheets API)**:
  ...
  - 시트 4개를 받았으나 1·2번은 비용/CAC 데이터(다른 컬럼), 3번만 신규콜 raw, 4번은 공유 미적용. 시트별 용도/매핑 미해결.
  - **⏳ 대기 중 (2026-05-16 예정)**: 사용자가 "각 시트의 어떤 컬럼/탭을 광고 대시보드의 어떤 지표에 매핑할지" 결정해서 알려준다고 함. 받으면 매핑 작업 진행.
  - 현 상태: `.env`의 `LEAD_SHEET_ID`에서 4번 시트 제외 (권한 문제). 1·2·3만 fetch 시도하지만 컬럼 구조 다르므로 실제 데이터 의미 있게 들어오는지는 미검증.
- **Google Ads 연동**:
